...
class report_filters
{
    ...
    static function set_filters($report)
    {
        ...
        {
            $report_page_filters[$report['id']]['filter_by_entity' . $i] = isset($_POST['filter_by_entity' . $i]) ? (int)$_POST['filter_by_entity' . $i] : 0;   
        }
    ...
    function render_fitler_by_entity()
    {
        ...
        for($i=1;$i<=self::COUNT_EXTRA_FILTERS;$i++)
        {
            ...
            <div class="form-group">
                ...
                <label style="width: 320px;">' . select_entities_tag('filter_by_entity' . $i, $choices, $value,['entities_id'=>$entity_id,'class'=>'report_page_' . $this->report['id'] . '_filter_by_entity' .$i]). '</label>    
            </div>    
            <script>
            $(function(){
