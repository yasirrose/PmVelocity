...
class blocks_table
{
        ...
        $("#report_page_' .  $this->block['id'] . '_chart").highcharts(         
            ...
            tooltip: {
                formatter: function () {
                    ...
                    if(this.x)
                    {
                        return \'<span style="font-size: 10px;">\' + this.x + \'</span><br><b>\' + this.series.name + \': </b>\' + this.point.y + (this.point.percentage ? " "+Math.round(this.point.percentage)+"%" : "");
                    }
                    else
                    {
    ...
    function prepare_chart_data($item_count,$item_row)
    {
    ...
    {        
        ...
        while($item = db_fetch_array($item_query))
        {
            ...
            if($this->has_chart())
            {
                $this->prepare_chart_data($item_count,$item_row);
            }
