...
{
	public $position, $entities_id, $reports, $number_of_rows_per_page, $current_page_number, $number_of_rows;
	...
	function render_report()
	{
		...
		if(!strlen($this->reports['related_entities_fields'])) return '';
    ...
    $html = '<div class="dataTables_paginate paging_bootstrap"><ul class="pagination">';
		...
		if ($cur_window_num > 1) $html .= '<li><a href="#" onClick="load_item_pivot_tables(' . $this->reports['id'] . ',' . (($cur_window_num - 1) * $max_page_links) . '); return false;" title=" ' . sprintf(PREVNEXT_TITLE_PREV_SET_OF_NO_PAGE, $max_page_links) . ' ">...</a></li>';
		...
		if ($cur_window_num < $max_window_num) $html .= '<li><a href="#"  onClick="load_item_pivot_tables(' . $this->reports['id'] . ',' . ($cur_window_num * $max_page_links + 1) . ')" title=" ' . sprintf(PREVNEXT_TITLE_NEXT_SET_OF_NO_PAGE, $max_page_links) . ' ">...</a></li>';
	...
	function build_table_rows($table_rows)
	{
		...
		{				
			...
			if($offset>$this->number_of_rows) $offset = $this->number_of_rows; 
		}		
