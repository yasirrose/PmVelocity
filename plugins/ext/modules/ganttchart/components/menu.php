...
{
	$reports_query = db_query("select g.* from app_ext_ganttchart g, app_entities e, app_ext_ganttchart_access ga where e.id=g.entities_id and g.id=ga.ganttchart_id and ga.access_groups_id='" . db_input($app_user['group_id']) . "' order by name");
}
else
{
	$reports_query = db_query("select g.* from app_ext_ganttchart g, app_entities e where e.id=g.entities_id order by g.name");
}
